...
test.describe("QR Label Redirect Middleware", () => {
  test("should redirect valid label (fresh-01) to product page", async ({
    page,
  }) => {
  ...
  test("should redirect valid terpene label (sleep-terpene-xyz) to product page", async ({
    page,
  }) => {
  ...
  test("should not redirect if no label parameter is present", async ({
    page,
  }) => {
    await page.goto("/");
